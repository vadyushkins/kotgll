...
class TestCFGGraphInputWithSPPFSuccess {
  @Test
  fun `test 'empty' hand-crafted grammar`() {
    ...
    val result = GLL(nonterminalS, listOf(graph)).parse()
  ...
  fun `test 'a' hand-crafted grammar`() {
    ...
    val result = GLL(nonterminalS, listOf(readGraphFromString("a"))).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
    ...
    assertEquals(expected = hashMapOf(0 to hashSetOf(1)), actual = pairs)
  }
  ...
  fun `test 'ab' hand-crafted grammar`() {
    ...
    val result = GLL(nonterminalS, listOf(readGraphFromString("ab"))).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
    ...
    assertEquals(expected = hashMapOf(0 to hashSetOf(2)), actual = pairs)
  }
  ...
  fun `test 'a-star' hand-crafted grammar`(input: String) {
    ...
    val result = GLL(nonterminalS, listOf(readGraphFromString(input))).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
  ...
  fun `test '(ab)-star' hand-crafted grammar`(input: String) {
    ...
    assertEquals(expected = hashMapOf(0 to hashSetOf(cur.id)), actual = pairs)
  }
  ...
  fun `test 'a-plus' hand-crafted grammar one cycle graph`() {
    ...
    val result = GLL(nonterminalS, listOf(graphNode0, graphNode1)).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
    ...
    assertEquals(expected = hashMapOf(0 to hashSetOf(1), 1 to hashSetOf(1)), actual = pairs)
  }
  ...
  fun `test 'a-star' hand-crafted grammar one cycle graph`() {
    ...
    assertEquals(
        expected = hashMapOf(0 to hashSetOf(0, 1), 1 to hashSetOf(1)),
        actual = pairs,
    )
  }
