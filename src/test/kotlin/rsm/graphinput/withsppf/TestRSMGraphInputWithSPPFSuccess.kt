...
class TestRSMGraphInputWithSPPFSuccess {
  ...
  fun `test 'a' hand-crafted grammar`() {
    ...
    val result = GLL(rsmState0, listOf(readGraphFromString("a"))).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
  ...
  fun `test 'ab' hand-crafted grammar`() {
    ...
    val result = GLL(rsmState0, listOf(readGraphFromString("ab"))).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
  ...
  fun `test 'ab or cd' ambiguous hand-crafted grammar`(input: String) {
    ...
    val result = GLL(rsmState0, listOf(graphNode0)).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
  ...
  fun `test 'dyck' hand-crafted grammar two cycle graph`() {
    ...
    val result = GLL(rsmState0, listOf(graphNode0, graphNode1, graphNode2, graphNode3)).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
  ...
  fun `test 'a-plus' hand-crafted grammar one cycle graph`() {
    ...
    val result = GLL(rsmState0, listOf(graphNode0, graphNode1)).parse()
    val pairs: HashMap<Int, HashSet<Int>> = HashMap()
    result.keys.forEach { tail ->
      if (!pairs.containsKey(tail)) pairs[tail] = HashSet()
      result[tail]!!.keys.forEach { head -> pairs[tail]!!.add(head) }
